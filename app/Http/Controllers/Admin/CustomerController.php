...
class CustomerController extends Controller
{
    ...
    public function edit($id , Request $request) {
        ...
        if ($customer) {
            ...
            $ward = $this->provinces->getWardByCode($request->input('ward'));
            $rule = [
                'email' => ['required', 'email', Rule::unique('customer')->ignore($customer->id)],
                'name' => 'required',
                'phone' => ['required', 'numeric', Rule::unique('customer')->ignore($customer->id)],
                'birth' => 'required|date',
                'gender' => 'required|in:1,2',
                'description' => 'nullable|min:5',
                'province' => ['required', function ($attribute, $value, $fail) use ($province) {
            ...
            $customer->birth = date('Y-m-d', strtotime($request->input('birth')));
            $customer->gender = $request->input('gender');
            $customer->description = $request->has('description') ? $request->input('description') : null;
