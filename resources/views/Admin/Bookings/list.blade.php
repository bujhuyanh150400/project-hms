...
@section('body')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class="bi bi-chevron-right"></i>
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Giờ khám của:
                        {{ $user->name }}</span>
                </div>
            </li>
        </ol>
    ...
    {{-- End: Navigation --}}
    {{-- Search --}}
    <form action="{{ route('bookings.list', ['user_id' => $user->id]) }}" method="GET" class="form-loading-submit">
        <div class="relative bg-white shadow-md rounded-lg mt-4 border p-4 ">
            ...
            <div class="flex justify-start items-end gap-3 ">
                ...
                <a href="{{ route('bookings.view_add', ['user_id' => $user->id]) }}" class="btn-custom btn-success"><i
                        class="bi bi-plus"></i>Taọ giờ khám</a>
            </div>
        </div>
    ...
    {{-- Data list --}}
    <div class="mt-9">
        @if (!empty($bookings) and count($bookings) > 0)
            <div class="grid grid-cols-4 gap-4">
                @foreach ($bookings as $booking)
                    ...
                    <div id="{{ $booking->id }}" role="tooltip"
                        ...
                        {{ $checkTime ? 'Nhấn để chỉnh sửa lịch khám' : 'Không thể sửa được lịch trong quá khứ' }}
