        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Danh sách phòng
                        khám</span>
                </div>
            </li>
        </ol>
            ...
            <div class="grid grid-cols-4 gap-3 mb-3">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" name="filter[keyword]" id="keyword"
                        value="{{ old('filter.keyword', $filter['keyword'] ?? '') }}" placeholder="Tìm kiếm theo tên cơ sở">
                </div>
                <div class="form-group">
                    <label class="form-label" for="active">Cơ sở hoạt động</label>
            ...
            <x-admin.table>
                ...
                <x-slot:body>
                    @foreach ($clinics as $clinic)
                        <tr>
                            <td>
                                <a class="hover:!text-blue-700 font-medium"
                                    href="{{ route('clinic.view', ['id' => $clinic->id]) }}">{{ $clinic->name }}</a>
                            </td>
                            <td>
                                <img class="w-32 h-32 rounded"
                                    @if (!empty($clinic->logo)) src="{{ route('file.show', ['filepath' => $clinic->logo]) }}"
                                    @else
                            ...
                            <td>
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('clinic.deleted', ['id' => $clinic->id]) }}" method="POST"
