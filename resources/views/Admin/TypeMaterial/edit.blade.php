...
@section('body')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class="bi bi-chevron-right"></i>
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Sửa loại vật tư:
                        {{ $type_material->name }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <form class="form-loading-submit" action="{{ route('type_material.edit', ['id' => $type_material->id]) }}"
        method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4 mb-4">
                <div class="form-group">
                    <label for="name" class=" @error('name') form-label-error @else form-label @enderror"> Tên loại
                        vật tư </label>
                    <input type="text" name="name" id="name" value="{{ old('name', $type_material->name) }}"
                ...
                <div class="form-group">
                    <label for="description" class=" @error('description') form-label-error @else form-label @enderror">Mô
                        tả</label>
                    <textarea class="ckeditor" name="description" id="description">{{ old('description', $type_material->description) }}</textarea>
                    @error('description')
                        <span class="form-alert">{{ $message }}</span>
                    @enderror
                </div>
