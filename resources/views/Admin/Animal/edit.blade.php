        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class="bi bi-chevron-right"></i>
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2">Sửa thú cưng</span>
                </div>
            </li>
        </ol>
    </nav>
    {{-- End: Navigation --}}
    <form class="form-loading-submit" action="{{ route('animal.edit', ['id' => $animal->id]) }}" method="POST"
        ...
        <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow ">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="age" min="0" max="30" id="age"
                               value="{{ old('age',$animal->age) }}" class="@error('age') form-input-error @else form-input @enderror"
                    ...
                    <div class="form-group">
                        ...
                        <select name="gender" id="gender"
                            ...
                            <option value="1" @if ((int) old('gender',$animal->gender) === 1) selected @endif>Đực</option>
                            <option value="2" @if ((int) old('gender',$animal->gender) === 2) selected @endif>Cái</option>
                        </select>
                        @error('gender')
                        <span class="form-alert">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" id="type"
                            ...
                            @foreach (TypeAnimal::getList() as $type)
                                <option value="{{ $type['value'] }}" @if ((int) old('type',$animal->type) === $type['value']) selected @endif>
                                    {{ $type['text'] }}</option>
                            @endforeach
                        </select>
                        @error('type')
                ...
                <div class="">
                    <div class="flex items-center justify-center flex-col gap-2">
                        ...
                        <div class="w-64 h-64 flex items-center justify-center border-dashed border-4 bg-gray-100 rounded ">
                            @if (!empty($animal->avatar))
                                <img class="w-full h-full select-none" id="avatar-preview"
                                     src="{{ route('file.show', ['filepath' => $animal->avatar]) }}" />
                            @else
                    ...
                    <div class="form-group mt-4">
                        ...
                        <textarea class="ckeditor" name="description" id="description">{{ old('description',$animal->description) }}</textarea>
                        @error('description')
                        <span class="form-alert">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="flex justify-end items-center gap-2">
                <a href="{{ route('animal.list') }}" class="btn-custom btn-default"><i class="bi bi-arrow-left"></i>Quay
                    lại</a>
                <button type="submit" class="btn-custom btn-success"><i class="bi bi-pen"></i>Sửa</button>
            </div>
        </div>
    </form>
@endsection
