    ...
    <div class="w-full bg-white border border-gray-200 rounded-lg shadow-sm mb-4 p-4">
        <div class="flex items-center justify-between ">
            <div class="flex items-center gap-4">
                <img class="w-24 h-24 rounded border border-gray-300 shadow-xl"  @if(!empty($user->avatar))
                ...
                <div class="flex items-start gap-2 flex-col">
                    ...
                    <span class="text-sm text-gray-500 dark:text-gray-400">Chức vụ: <span class="font-bold text-sm">@isset(PermissionAdmin::getList()[$user->permission]){{ PermissionAdmin::getList()[$user->permission]['text'] }}@endisset</span></span>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{route('users.list')}}" class="btn-custom btn-default"><i class="bi bi-arrow-left"></i> Quay lại</a>
                <a href="{{route('users.view_edit',['id'=>$user->id])}}" class="btn-custom btn-primary"><i class="bi bi-pencil-square"></i> Chỉnh sửa</a>
            </div>
        </div>
    </div>
